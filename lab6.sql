...
--'math' or 'science'. Ensure that their city does not start with 'B'.
	...
	WHERE FIRSTNAME LIKE 'B%' AND 
		  LASTNAME LIKE '%A' AND 
		  WEBSITE LIKE '%MATH%' OR WEBSITE LIKE '%SCIENCE%' AND
		  CITY NOT LIKE 'B%'
...
--19. Retrieve students who have 'Shah' in their last name and whose city ends with 'd'. Additionally, their website should be either null or contain 'com'.
	...
	WHERE LASTNAME LIKE '%SHAH%' AND
		  CITY LIKE '%D' AND
		  WEBSITE LIKE NULL OR WEBSITE LIKE '%COM%'
...
--20. Select students whose first and second character is a vowel. Their city should start with 'R' and they must have a website containing '.com'. 
	SELECT * FROM STUDENT
	WHERE FIRSTNAME LIKE '[A,E,I,O,U][A,E,I,O,U]%' AND
		  CITY LIKE 'R%' AND 
		  WEBSITE LIKE '%.COM%'
...
--must have a website that ends with '.com' and their first name should not start with 'A'.
	...
	WHERE ADDRESS LIKE '%-%' AND 
		  (CITY LIKE 'R%' OR CITY LIKE 'B%') AND
...
--quotes. Their last name should start with 'P' and they must have a website that contains 'on'.
	...
	WHERE CITY NOT LIKE '%S%' AND
		  (ADDRESS LIKE '%''%' OR ADDRESS LIKE '%"%') AND
		  LASTNAME LIKE 'P%' AND
		  WEBSITE LIKE '%ON%'
